...
parser.add_argument('--checkpoint', type=str, default='meta-llama/Llama-3.1-8B-Instruct', help='model checkpoint')
...
def generate_text(
    ...
    for k, prompt in zip(index_k, prompts):
        ...
        if hidden_use == "only_first" or hidden_use == "all":
            ...
            hidden_first = outputs.hidden_states[0][-1].view(-1, model.config.hidden_size) #extract the first token's hidden states from the last layer
        ...
        if hidden_use == "except_first" or hidden_use == "all":
            ...
            hidden_except_first = torch.stack([item[-1] for item in outputs.hidden_states[1:]]).view(-1, model.config.hidden_size)
            torch.save(hidden_except_first, f"{save_hidden}/hidden_{k}.pt")
        
        if hidden_use == 'only_last':
            #saving the last hidden states
            hidden_last = outputs.hidden_states[-1][-1].view(-1,model.config.hidden_size)
